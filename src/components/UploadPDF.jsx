...
export default function UploadPDF({ onFilesSelect, selectedFiles, onPreview }) {
  ...
  const removeFile = (index, e) => {
    ...
    // Ensure input is cleared if no files remain (though we clear on change now too)
    ...
    <div className="bg-white rounded-3xl border border-zinc-100 shadow-xl shadow-zinc-200/50 p-6">
      ...
      <AnimatePresence mode="wait">
        {!hasFiles ? (
          ...
          >
            <div className={`h-20 w-20 rounded-[2rem] flex items-center justify-center mb-6 transition-all duration-500 ${
              isDragging ? 'bg-indigo-600 text-white scale-110 rotate-12' : 'bg-zinc-50 text-zinc-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 group-hover:scale-110'
            }`}>
              <Upload size={32} />
            </div>
            <p className={`text-sm font-bold uppercase tracking-widest transition-colors ${isDragging ? 'text-indigo-600' : 'text-zinc-500 group-hover:text-indigo-600'}`}>
              ...
              >
                ...
                >
                  ...
                  <div className="overflow-hidden">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-bold text-zinc-900 truncate group-hover:text-indigo-600 transition-colors">{file.name}</p>
                      <span className="text-[8px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-md border border-emerald-100 uppercase tracking-tighter">Ready</span>
                    </div>
                ...
                <div className="flex items-center gap-1 sm:gap-2 ml-2 shrink-0">
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider hidden xs:inline">Preview</span>
                  </button>
                ...
                {/* Desktop Hover Overlay - Subtle hint */}
